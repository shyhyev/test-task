<div x-data="{ type: 'sign-in' }" class="w-full max-w-md">
    <div class="flex items-center justify-center mt-6">
        <a href="#"
           @click="type = 'sign-in'"
           class="w-1/3 pb-4 font-medium text-center capitalize"
           :class="type === 'sign-in' ? 'border-b-2 border-blue-500 text-gray-800' : 'text-gray-500 border-b'">
            sign in
        </a>
        ...
        <a href="#"
           @click="type = 'sign-up'"
           class="w-1/3 pb-4 font-medium text-center capitalize"
           :class="type === 'sign-up' ? 'border-b-2 border-blue-500 text-gray-800' : 'text-gray-500 border-b'">
            sign up
        </a>
    </div>

    <form wire:submit="login" x-show="type === 'sign-in'">
        @if($errors->any())
        ...
        <div class="mt-6">
            <button class="cursor-pointer w-full px-6 py-3 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-blue-500 rounded-lg hover:bg-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-50">
                Sign In
            </button>
        </div>
    </form>
    <form wire:submit="register" x-show="type === 'sign-up'">
        @if($errors->any())
            <div class="flex my-3 w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-md">
                <div class="flex items-center justify-center w-12 bg-red-500">
                    <svg class="w-6 h-6 text-white fill-current" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
